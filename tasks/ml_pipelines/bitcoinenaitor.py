...
class Bitcoinenaitor(BaseTask):
    def __init__(self, name: str, frequency: timedelta, config: dict[str, Any]):
        ...
        self.experiment_name = config.get("mlflow_config", {}).get(
            "experiment_name", f"{self.connector_name}_{self.trading_pair}_{self.interval}"
        )
    ...
    async def execute(self):
        ...
        start_time = pd.Timestamp(time.time() - self.days_for_training * 24 * 60 * 60, unit="s").tz_localize(UTC).timestamp()
        logging.info(f"{now} - Start date: {start_time}, End date: {end_time}")
        await self.timescale_client.connect()
        candles = await self.timescale_client.get_candles_last_days(
            self.connector_name, self.trading_pair, self.interval, self.days_for_training
        )
        df = candles.data
        df["side"] = 1
        logging.info("Starting meta labeling")
        df_with_tbm = triple_barrier_method(
            df,
            tp=self.target_multiplier,
            sl=self.target_multiplier,
            tl=self.tl,
            std_span=self.std_span,
            trade_cost=self.trade_cost,
        )
        logging.info("Meta labeling complete, adding features ")
    ...
    def train_and_evaluate_model(self, df_processed, scaler):
        ...
        with mlflow.start_run(
            ...
            run_name=f"{self.connector_name}_{self.trading_pair}_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
        ):
            # Log parameters
            mlflow.log_params(
                {
            ...
            mlflow.log_metrics({"final_accuracy": accuracy, "final_precision": precision, "final_recall": recall, "final_f1": f1})

            # Log confusion matrix as a figure
            cm = confusion_matrix(y_test, y_pred)
            cm_dict = {"matrix": cm.tolist(), "labels": sorted(list(set(y_test)))}
            ...
            feature_importance = {
                feature: importance for feature, importance in zip(feature_columns, model.feature_importances_, strict=False)
            }
...
async def main(config):
    ml_pipeline = Bitcoinenaitor(name="Candles Downloader", frequency=timedelta(hours=1), config=config)
...
if __name__ == "__main__":
    ...
    model_params = {"n_estimators": 500, "max_depth": 3, "cv_folds": 5}
