...
class TradesDownloaderTask(BaseTask):
    ...
    async def execute(self):
        ...
        start_time = pd.Timestamp(self.start_time, unit="s").tz_localize(UTC)
        ...
        trading_pairs = (
            trading_rules.filter_by_quote_asset(self.quote_asset)
        ...
        for i, trading_pair in enumerate(trading_pairs):
            ...
            try:
                ...
                last_trade_id = await timescale_client.get_last_trade_id(
                    connector_name=self.connector_name, trading_pair=trading_pair, table_name=table_name
                )
                trades = await self.clob.get_trades(
                    self.connector_name, trading_pair, int(start_time.timestamp()), int(end_time.timestamp()), last_trade_id
                )
                ...
                await timescale_client.delete_trades(
                    connector_name=self.connector_name, trading_pair=trading_pair, timestamp=cutoff_timestamp
                )
                # TODO: isolate resampling and metrics management in another module
                # TODO: pass list of intervals to perform better
                await timescale_client.compute_resampled_ohlc(
                    connector_name=self.connector_name, trading_pair=trading_pair, interval="1s"
                )
