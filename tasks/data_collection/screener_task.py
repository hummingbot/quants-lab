...
class MarketScreenerTask(BaseTask):
    def __init__(self, name: str, frequency: timedelta, config: dict[str, Any], ts_client: TimescaleClient = None):
        ...
        self.interval_mapping = {"1m": "one_min", "3m": "three_min", "5m": "five_min", "15m": "fifteen_min", "1h": "one_hour"}
    ...
    async def process_pair(self, connector_name, trading_pair):
        ...
        try:
            ...
            screener_metrics = self.calculate_global_screener_metrics(
                candles_df=candles.data, connector_name=connector_name, trading_pair=trading_pair
            )
            ...
            screener_metrics = {
                key: json.dumps(value) if isinstance(value, dict) else value for key, value in screener_metrics.items()
            }
    ...
    async def calculate_interval_metrics(self, connector_name, trading_pair):
        ...
        for selected_interval in self.intervals:
            ...
            try:
                ...
                logging.exception(f"{self.now()} - Error processing interval {selected_interval} for {trading_pair}\n {e}")
...
async def main():
    config = {"intervals": ["1m", "3m", "5m", "15m", "1h"]}
