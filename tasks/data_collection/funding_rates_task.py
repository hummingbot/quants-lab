        ...
        try:
            ...
            for connector_name in self.config.get("connector_names", ["binance_perpetual"]):
                ...
                await self.mongo_client.insert_documents(
                    ...
                    index=[("trading_pair", 1), ("connector_name", 1), ("next_funding_utc_timestamp", 1)],
                )
                ...
                await self.mongo_client.insert_documents(
                    ...
                    documents=results,
                    index=[("timestamp", 1), ("pair1", 1), ("pair2", 1)],
                )
...
async def main():
    ...
    task = FundingRatesTask(name="funding_rate_task", frequency=timedelta(hours=1), config=task_config)
    await task.execute()
