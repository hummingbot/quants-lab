...
class CandlesDownloaderTask(BaseTask):
    ...
    async def execute(self):
        ...
        logging.info(f"{now} - Starting candles downloader for {self.connector_name} at {time.strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        for i, trading_pair in enumerate(trading_pairs):
            for interval in self.intervals:
                ...
                try:
                    ...
                    await timescale_client.append_candles(table_name=table_name, candles=candles.data.values.tolist())
...
async def main(config):
    candles_downloader_task = CandlesDownloaderTask(name="Candles Downloader", frequency=timedelta(hours=1), config=config)
