        ...
        for connector_name, trading_pair, interval in available_candles:
            ...
            try:
                ...
                candles = await timescale_client.get_all_candles(
                    connector_name=self.config["connector_name"], trading_pair=trading_pair, interval=interval
                )
                ...
                candles_df.to_parquet(filepath, engine="pyarrow", compression="snappy", index=True)
...
async def main(config):
    candles_export_task = LocalCacheExportTask(name="Candles Exporter", frequency=timedelta(hours=1), config=config)
    await candles_export_task.execute()


if __name__ == "__main__":
    timescale_config = {"host": "localhost", "port": 5432, "user": "admin", "password": "admin", "database": "timescaledb"}
    config = {
