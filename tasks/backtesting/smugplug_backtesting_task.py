...
class BacktestingTask(BaseTask):
    ...
    async def execute(self):
        ...
        logger.info(f"Optimizing strategy for top markets: {top_markets_df.shape[0]}")
        for index, row in top_markets_df.iterrows():
            ...
            config_generator = SmugPlugConfigGenerator(
                start_date=start_date, end_date=end_date, backtester=DirectionalTradingBacktesting()
            )
            ...
            candles = await optimizer._db_client.get_candles(
                connector_name, trading_pair, resolution, start_date.timestamp(), end_date.timestamp()
            )
            ...
            await optimizer.optimize(study_name=f"smugplug_test_task_{today_str}", config_generator=config_generator, n_trials=50)
