...
class RouterResult(BaseModel):
    ...
    price_impact_pct: str | None = Field(None, alias="priceImpactPercentage")
    quote_compare_list: list[QuoteCompare] = Field(..., alias="quoteCompareList")
    to_token: TokenInfo = Field(..., alias="toToken")
    to_token_amount: str = Field(..., alias="toTokenAmount")
    trade_fee: str = Field(..., alias="tradeFee")
    origin_to_token_amount: str | None = Field(None, alias="originToTokenAmount")
    ...
    def best_venue(self) -> QuoteCompare:
        ...
        return min(self.quote_compare_list, key=lambda x: x.get_price(self.from_amount_decimal))

    def get_venue_prices(self) -> dict[str, Decimal]:
        """Get a mapping of venue names to their prices (input/output)"""
        return {quote.dex_name: quote.get_price(self.from_amount_decimal) for quote in self.quote_compare_list}
...
class SwapTransaction(BaseModel):
    ...
    gas: str
    gas_price: str = Field(..., alias="gasPrice")
    max_priority_fee_per_gas: str | None = Field(None, alias="maxPriorityFeePerGas")
    to: str
    value: str
    min_receive_amount: str | None = Field(None, alias="minReceiveAmount")
...
class OKXDexAPI:
    ...
    async def approve_transaction(self, chain_id: str, token_contract_address: str, approve_amount: str) -> ApproveResponse:
        """
        ...
        """
        params = {"chainId": chain_id, "tokenContractAddress": token_contract_address, "approveAmount": approve_amount}
        response = await self.get(self.APPROVE_TRANSACTION, params)
        return ApproveResponse(**response)

    async def get_quote(
        self, chain_id: str, from_token_address: str, to_token_address: str, amount: str, fee_percent: str | None = None
    ) -> QuoteResponse:
        """
    ...
    async def swap(
        self, chain_id: str, from_token_address: str, to_token_address: str, amount: str, slippage: str, user_wallet_address: str
    ) -> SwapResponse:
        """
    ...
    async def poll_for_confirmation(self, tx_sig: str, sleep_seconds: float = 0.5):
        await self.solana_client.confirm_transaction(tx_sig=tx_sig, commitment="confirmed", sleep_seconds=sleep_seconds)
    ...
    async def execute_solana_swap_via_okx(
        ...
        try:
            ...
            order_id = await self.broadcast_transaction(signed_tx=signed_tx_str, chain_index="501", address=wallet_address)
            return order_id
