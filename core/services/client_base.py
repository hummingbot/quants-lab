...
class ClientBase:
    ...
    async def post(self, endpoint: str, payload: Optional[Dict] = None, params: Optional[Dict] = None):
        ...
        response: ClientResponse = await self.session.post(url, json=payload, params=params, headers=headers)
    ...
    async def get(self, endpoint: str, params: Optional[Dict] = None):
        ...
        await self._ensure_session()
        url = f"{self.base_url}/{endpoint}"
        response = await self.session.get(url, params=params, headers={"accept": "application/json"})
        return await self._process_response(response)
