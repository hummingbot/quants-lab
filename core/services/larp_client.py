...
class LarpClient(ClientBase):
    ...
    # Solana endpoints
    async def get_balance(self, address: str | None = None, symbols: list[str] = ["SOL"]):
    ...
    # Jupiter endpoints
    async def get_jupiter_quote_swap(
        self,
    ...
    async def execute_jupiter_swap(
        self, input_token_symbol: str, output_token_symbol: str, amount: float, slippage_pct: float = 1
    ):
    ...
    # Orca endpoints
    async def get_orca_positions_owned(self, address: str | None = None):
    ...
    async def get_orca_bundles_owned(self, address: str | None = None):
    ...
    async def get_orca_quote_swap(
        self, input_token_symbol: str, output_token_symbol: str, amount: float, slippage_pct: float = 1, tick_spacing: int = 64
    ):
    ...
    async def execute_orca_swap(
        self, input_token_symbol: str, output_token_symbol: str, amount: float, tick_spacing: int = 64, slippage_pct: float = 1
    ):
    ...
    async def add_liquidity_quote(self, position_address: str, quote_token_amount: float, slippage_pct: float = 1):
        ...
        payload = {"positionAddress": position_address, "quoteTokenAmount": quote_token_amount, "slippagePct": slippage_pct}
        return await self.post(endpoint, payload=payload)
    ...
    async def remove_liquidity(self, position_address: str, percentage_to_remove: float, slippage_pct: float = 1):
        ...
        payload = {"positionAddress": position_address, "percentageToRemove": percentage_to_remove, "slippagePct": slippage_pct}
        return await self.post(endpoint, payload=payload)
    ...
    async def open_orca_positions_in_bundle(
        self,
        base_symbol: str,
        quote_symbol: str,
        tick_spacing: int,
    ...
    async def add_liquidity_in_bundle(
        self, position_bundle_address: str, quote_token_amounts: list[float], slippage_pct: float = 1
    ):
    ...
    async def remove_liquidity_in_bundle(self, position_bundle_address: str, percentages: list[float], slippage_pct: float = 1):
        """Remove liquidity from multiple Orca positions in a bundle."""
        endpoint = "orca/remove-liquidity-in-bundle"
        payload = {"positionBundleAddress": position_bundle_address, "percentages": percentages, "slippagePct": slippage_pct}
        return await self.post(endpoint, payload=payload)
