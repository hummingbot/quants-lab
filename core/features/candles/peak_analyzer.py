...
class PeakAnalyzer:
    ...
    def get_peaks_and_clusters(
        ...
        for i in range(intervals):
            ...
            if close_price_filter:
                ...
                high_clusters, _ = (
                    self._hierarchical_clustering(filtered_high_peaks, num_clusters)
                    if len(filtered_high_peaks) > num_clusters
                    else [],
                    [],
                )
                low_clusters, _ = (
                    self._hierarchical_clustering(filtered_low_peaks, num_clusters)
                    if len(filtered_low_peaks) > num_clusters
                    else [],
                    [],
                )
    ...
    def _get_last_valid_clusters(
        self, clusters: list[dict], current_index: int, cluster_type: str, close_price: float
    ) -> list[float]:
