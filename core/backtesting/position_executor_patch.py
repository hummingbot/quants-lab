# Utility to patch PositionExecutorSimulator.simulate with label-based slicing
...
def _patched_simulate(
    self: PositionExecutorSimulator, df: pd.DataFrame, config: PositionExecutorConfig, trade_cost: float
) -> ExecutorSimulation:
    ...
    first_trailing_sl_timestamp = (
        ...
        if trailing_sl_delta_pct and trailing_sl_trigger_pct
        else None
    )
    close_timestamp = min(
        [
...
def patch_position_executor_simulator() -> None:
    """Patch PositionExecutorSimulator.simulate with the pandas label slicing implementation."""
    PositionExecutorSimulator.simulate = _patched_simulate


__all__ = ["patch_position_executor_simulator"]
