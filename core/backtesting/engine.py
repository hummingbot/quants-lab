...
class BacktestingEngine:
    def __init__(
        self, load_cached_data: bool = True, root_path: str = "", custom_backtester: BacktestingEngineBase | None = None
    ):
    ...
    def _load_candles_cache(self, root_path: str):
        ...
        for file in all_files:
            ...
            try:
                ...
                self._bt_engine.backtesting_data_provider.candles_feeds[f"{connector_name}_{trading_pair}_{interval}"] = candles
        ...
        for file in all_files:
            ...
            try:
                if connector_name in file and trading_pair in file:
                    ...
                    for column in columns:
                        candles[column] = pd.to_numeric(candles[column])
                    self._bt_engine.backtesting_data_provider.candles_feeds[f"{connector_name}_{trading_pair}_{interval}"] = (
                        candles
                    )
            except Exception as e:
                logger.error(f"Error loading {file}: {e}")
    ...
    async def run_backtesting(
        self, config: ControllerConfigBase, start: int, end: int, backtesting_resolution: str, trade_cost: float = 0.0006
    ) -> BacktestingResult:
