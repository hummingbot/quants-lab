...
class BacktestingResult(DataStructureBase):
    ...
    def get_results_summary(self, results: dict | None = None):
        if results is None:
                ...
                if executor.net_pnl_quote > Decimal(0):
                    fig.add_trace(
                        go.Scatter(
                            ...
                            mode="lines",
                            showlegend=False,
                            line=dict(color="green", width=2, dash=line_style if line_style == "dash" else None),
                            name=name,
                        ),
                        row=row,
                        col=col,
                    ...
                    fig.add_trace(
                        go.Scatter(
                            ...
                            mode="lines",
                            showlegend=False,
                            line=dict(color="red", width=2, dash=line_style if line_style == "dash" else None),
                            name=name,
                        ),
                        row=row,
                        col=col,
