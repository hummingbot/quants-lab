...
class TaskRunner:
    ...
    def get_common_config(self) -> dict[str, Any]:
        ...
        return {
            ...
            "mongo_config": {"uri": os.getenv("MONGO_URI"), "db": os.getenv("MONGO_DB")},
        }
    ...
    def initialize_tasks(self) -> list[BaseTask]:
        ...
        for task_name, task_config in self.tasks_config["tasks"].items():
            ...
            try:
                ...
                task = task_class(name=task_name, frequency=timedelta(hours=task_config["frequency_hours"]), config=config)
                tasks.append(task)
