...
class PeinjoControllerConfig(DirectionalTradingControllerConfigBase):
    ...
    length: int = Field(
        default=200,
        client_data=ClientFieldData(prompt=lambda mi: "Enter the length of the mean reversion channel: ", prompt_on_new=True),
    )
    inner_mult: float = Field(
        default=1.0,
        client_data=ClientFieldData(
            prompt=lambda mi: "Enter the inner multiplier of the mean reversion channel: ", prompt_on_new=True
        ),
    )
    outer_mult: float = Field(
        default=2.415,
        client_data=ClientFieldData(
            prompt=lambda mi: "Enter the outer multiplier of the mean reversion channel: ", prompt_on_new=True
        ),
    )
    source: str = Field(
        default="hlc3",
        client_data=ClientFieldData(prompt=lambda mi: "Enter the source of the mean reversion channel: ", prompt_on_new=True),
    )
    filter_type: str = Field(
        default="SuperSmoother",
        client_data=ClientFieldData(
            prompt=lambda mi: "Enter the filter type of the mean reversion channel: ", prompt_on_new=True
        ),
    )
