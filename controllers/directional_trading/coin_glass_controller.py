...
import pandas as pd
from coin_glass_data_provider import CoinGlassDataProvider
...
class CoinGlassControllerConfig(ControllerConfigBase):
    """
    This class represents the configuration required to run a Directional Strategy with CoinGlass data provider.
    """

    controller_name: str = "coin_glass_controller"
    ...
    max_executors_per_side: int = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=True,
            prompt=lambda mi: "Enter the maximum number of executors per side (e.g., 1):",
        ),
    )
    ...
    host: str = Field(
        default="localhost",
        client_data=ClientFieldData(
            prompt=lambda mi: "Enter the database host (e.g., localhost): ",
            prompt_on_new=True,
        ),
    )
    port: int = Field(
        default=5432,
        client_data=ClientFieldData(
            prompt=lambda mi: "Enter the database port (e.g., 5432): ",
            prompt_on_new=True,
        ),
    )
    user: str = Field(
        default="admin",
        client_data=ClientFieldData(
            prompt=lambda mi: "Enter the database user (e.g., admin): ",
            prompt_on_new=True,
        ),
    )
    password: str = Field(
        default="admin",
        client_data=ClientFieldData(
            prompt=lambda mi: "Enter the database password (e.g., admin): ",
            prompt_on_new=True,
        ),
    )
    database: str = Field(
        default="timescaledb",
        client_data=ClientFieldData(
            prompt=lambda mi: "Enter the database name (e.g., timescaledb): ",
            prompt_on_new=True,
        ),
    )
...
class CoinGlassController(ControllerBase):
    """
    This class represents the base class for a Directional Strategy.
    """

    def __init__(self, config: CoinGlassControllerConfig, *args, **kwargs):
        ...
        self.coin_glass_data_provider: CoinGlassDataProvider = CoinGlassDataProvider(
            host=config.host,
            port=config.port,
            user=config.user,
    ...
    def create_actions_proposal(self) -> List[ExecutorAction]:
        ...
        if signal != 0 and self.can_create_executor(signal):
            ...
            amount = (
                ...
                / price
                / Decimal(self.config.max_executors_per_side)
            )
    ...
    def can_create_executor(self, signal: int) -> bool:
        ...
        active_executors_by_signal_side = self.filter_executors(
            ...
            and (x.side == TradeType.BUY if signal > 0 else x.side == TradeType.SELL),
        )
        max_timestamp = max(
            [executor.timestamp for executor in active_executors_by_signal_side],
            default=0,
        )
        active_executors_condition = (
            len(active_executors_by_signal_side) < self.config.max_executors_per_side
        )
        cooldown_condition = (
            self.market_data_provider.time() - max_timestamp > self.config.cooldown_time
        )
