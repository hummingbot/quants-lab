...
class RAJReversionControllerConfig(DirectionalTradingControllerConfigBase):
    ...
    close_alma_length: int = Field(
        default=80, client_data=ClientFieldData(prompt=lambda mi: "Enter the close ALMA length: ", prompt_on_new=True)
    )
    close_alma_offset: float = Field(
        default=0.85, client_data=ClientFieldData(prompt=lambda mi: "Enter the close ALMA offset: ", prompt_on_new=True)
    )
    close_alma_sigma: int = Field(
        default=16, client_data=ClientFieldData(prompt=lambda mi: "Enter the close ALMA sigma: ", prompt_on_new=True)
    )

    # Pivot Parameters
    pivot_left: int = Field(
        default=7, client_data=ClientFieldData(prompt=lambda mi: "Enter the left pivot length: ", prompt_on_new=True)
    )
    pivot_right: int = Field(
        default=7, client_data=ClientFieldData(prompt=lambda mi: "Enter the right pivot length: ", prompt_on_new=True)
    )
    array_percent: float = Field(
        default=86, client_data=ClientFieldData(prompt=lambda mi: "Enter the percentile threshold: ", prompt_on_new=True)
    )

    # ALMA over mean Parameters
    diff_alma_length: int = Field(
        default=9, client_data=ClientFieldData(prompt=lambda mi: "Enter the diff ALMA length: ", prompt_on_new=True)
    )
    diff_alma_offset: float = Field(
        default=0.85, client_data=ClientFieldData(prompt=lambda mi: "Enter the diff ALMA offset: ", prompt_on_new=True)
    )
    diff_alma_sigma: int = Field(
        default=16, client_data=ClientFieldData(prompt=lambda mi: "Enter the diff ALMA sigma: ", prompt_on_new=True)
    )

    percentile_rolling_window: int = Field(
        default=100,
        client_data=ClientFieldData(
            prompt=lambda mi: "Enter the rolling window for percentile calculation: ", prompt_on_new=True
        ),
    )
...
class RAJReversionController(DirectionalTradingControllerBase):
    def __init__(self, config: RAJReversionControllerConfig, *args, **kwargs):
        self.config = config
        self.max_records = (
            max(self.config.close_alma_length, self.config.pivot_left + self.config.pivot_right, self.config.diff_alma_length)
            + 20
    ...
    async def update_processed_data(self):
        ...
        # Calculate source
        df["src"] = np.where(
            ((df["high"] - df["alma"]) > abs(df["low"] - df["alma"])) & ((df["high"] - df["open"]) > abs(df["low"] - df["open"])),
            df["high"],
            np.where(
        ...
        pivot_highs_idx, pivot_highs, pivot_lows_idx, pivot_lows = self.calculate_pivots(
            df=df, source_column="diff", left=self.config.pivot_left, right=self.config.pivot_right
        )
