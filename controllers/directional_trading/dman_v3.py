...
class DManV3ControllerConfig(DirectionalTradingControllerConfigBase):
    ...
    bb_length: int = Field(default=100, json_schema_extra={"prompt": "Enter the Bollinger Bands length: ", "prompt_on_new": True})
    ...
    dynamic_order_spread: bool = Field(
        default=None, json_schema_extra={"prompt": "Do you want to make the spread dynamic? (Yes/No) ", "prompt_on_new": True}
    )
    dynamic_target: bool = Field(
        default=None, json_schema_extra={"prompt": "Do you want to make the target dynamic? (Yes/No) ", "prompt_on_new": True}
    )
