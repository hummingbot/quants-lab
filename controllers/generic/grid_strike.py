...
class GridStrikeConfig(ControllerConfigBase):
    ...
    grid_ranges: list[GridRange] = Field(
        default=[GridRange(id="R0", start_price=Decimal("40000"), end_price=Decimal("60000"), total_amount_pct=Decimal("0.1"))],
        client_data=ClientFieldData(is_updatable=True),
    )
    position_mode: PositionMode = PositionMode.HEDGE
    leverage: int = 1
    time_limit: int | None = Field(default=60 * 60 * 24 * 2, client_data=ClientFieldData(is_updatable=True))
    activation_bounds: Decimal = Field(default=Decimal("0.01"), client_data=ClientFieldData(is_updatable=True))
    min_spread_between_orders: Decimal | None = Field(default=None, client_data=ClientFieldData(is_updatable=True))
    min_order_amount: Decimal | None = Field(default=Decimal("1"), client_data=ClientFieldData(is_updatable=True))
...
class GridStrike(ControllerBase):
    ...
    def _calculate_grid_config(self):
        self.trading_rules = self.market_data_provider.get_trading_rules(self.config.connector_name, self.config.trading_pair)
        grid_levels = []
            ...
            if grid_range.active:
                ...
                for i, price in enumerate(prices):
                    ...
                    amount_quantized = self.market_data_provider.quantize_order_amount(
                        self.config.connector_name, self.config.trading_pair, amount_quote / self.get_mid_price()
                    )
    ...
    def active_executors(self, is_trading: bool) -> list[ExecutorInfo]:
        return [executor for executor in self.executors_info if executor.is_active and executor.is_trading == is_trading]
    ...
    def determine_create_executor_actions(self) -> list[ExecutorAction]:
        ...
        active_executors = (
            self.processed_data["active_executors_order_placed"] + self.processed_data["active_executors_order_trading"]
        )
        active_executors_level_id = [executor.custom_info["level_id"] for executor in active_executors]
        levels_allowed = sorted(
            [level for level in levels_allowed if level.id not in active_executors_level_id],
            ...
            create_actions.append(
                CreateExecutorAction(
                    ...
                    executor_config=PositionExecutorConfig(
                        ...
                        side=level.side,
                        level_id=level.id,
                        activation_bounds=[self.config.activation_bounds, self.config.activation_bounds],
    ...
    def determine_stop_executor_actions(self) -> list[ExecutorAction]:
        ...
        active_executor_of_non_active_ranges = [
            ...
            if executor.is_active and executor.custom_info["level_id"].split("_")[0] in non_active_ranges
        ]
        long_executors_to_stop = [
            executor.id
            for executor in active_executors_order_placed
            if executor.side == TradeType.BUY and executor.config.entry_price <= long_activation_bounds
        ]
        short_executors_to_stop = [
            executor.id
            for executor in active_executors_order_placed
            if executor.side == TradeType.SELL and executor.config.entry_price >= short_activation_bounds
        ]
        executors_id_to_stop = set(active_executor_of_non_active_ranges + long_executors_to_stop + short_executors_to_stop)
        return [StopExecutorAction(controller_id=self.config.id, executor_id=executor) for executor in list(executors_id_to_stop)]
