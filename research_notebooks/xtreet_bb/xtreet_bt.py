...
class XtreetBacktesting(DirectionalTradingBacktesting):
    def prepare_market_data(self) -> pd.DataFrame:
        ...
        df["signal"] = 0
        long_condition = df["close"] >= df[f"BBU_{self.controller.config.bb_length}_{self.controller.config.bb_std}"]
        short_condition = df["close"] <= df[f"BBL_{self.controller.config.bb_length}_{self.controller.config.bb_std}"]
