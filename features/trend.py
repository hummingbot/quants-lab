...
class Trend(FeatureBase[TrendConfig]):
    def calculate(self, candles):
        ...
        candles['trend_score'] = self.calculate_trending_score(candles['close'], short_window, long_window)

        return candles

    def calculate_trending_score(self, series: pd.Series, short_window: int, long_window: int) -> pd.Series:
